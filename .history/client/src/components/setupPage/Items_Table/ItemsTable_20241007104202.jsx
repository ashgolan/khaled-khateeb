...
export default function ItemsTable({
  ...
  const [hideProductquantitiesDiv, setHideProductquantitiesDiv] = useState(true)
  useEffect(() => {
    const getData = async () => {
    ...
    <>
      <form className="form-container-in-table">
        ...
        >
          ...
          {itemsValues?.product?.length > 0 && !changeStatus.disabled )|| !hideProductquantitiesDiv && (
            <div className="Productquantities">
              ...
              <button onClick={()=> setHideProductquantitiesDiv(true)}>hide</button>
            </div>
          )}
          {collReq !== "/clients" && (
            <input
              ...
              onChange={(e) => {
                setItemsValues((prev) => {
                  return {
                    ...
                    totalAmount:
                    +e.target.value + prev.sumOfPrices
                  };
                });
              }}
            ></input>
